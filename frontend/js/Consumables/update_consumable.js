document.addEventListener('DOMContentLoaded', () => {
    ...
    if (toggleCheckbox) {
      ...
      // Inicializar Choices.js después de llenar las opciones
        ...
        fetch('http://localhost:5501/consumables/id') // ⬅️ hago un fetch para traer la lista de IDS
          .then(res => res.json()) // ⬅️ Aqui estoy trallendo el paquete json
          .then(ids => {
            const todosLosIds = ids.insumos; // Array con todos los IDs
        ...
        cropSelect.addEventListener('change', () => {  // ⬅️ Para cuando se selecciona otro ID se cambie el valor de la variable currentID
          ...
          fetch(`http://localhost:5501/api/consumables/${id}`) // ⬅️ Por medio del ID traigo la columna correspondiente( anteriormente la traje tambien, pero para cargar el select, ahora es para los inputs)
            .then(res => {
            ...
            .then(data => {  // ⬅️ Los muestro por aqui
              ...
              cropForm.estado_insumo.value = data.state_consumables;
      ...
      if (cropForm) {
        cropForm.addEventListener('submit', (e) => { // ⬅️ Cuando se haga el sumbit
          ...
          data.append("tipo_insumo", cropForm.tipo_insumo.value);
          data.append("nombre_insumo", cropForm.nombre_insumo.value);
          data.append("cantidad_insumo", cropForm.cantidad_insumo.value);
          data.append("unidad_insumo", cropForm.unidad_insumo.value);
          data.append("unidad_valor", cropForm.unidad_valor.value);
          data.append("total_valor", cropForm.total_valor.value);
          data.append("descripcion_insumo", cropForm.descripcion_insumo.value);
          data.append("estado_insumo", cropForm.estado_insumo.value);
  
    
          fetch(`http://localhost:5501/api/consumables-actualizar/${currentID}`, { // ⬅️ Mandamos con fetch la actualizacion con su id correspondiente
            method: 'PUT',
            body: data
          })
          .then(res => res.json())
