...
document.addEventListener('DOMContentLoaded', () => { // ⬅️ Esperamos a que cargue el DOM
  ...
  async function obtenerinsumos(pagina = 1, buscar = '') {
    try {
      ...
      console.error('Error al obtener insumos:', err.message); // ⬅️ Mensaje de error
    }
  }
  


  function mostrarinsumos(insumos) { // ⬅️ Esta es la funcion para mostrar en una tabla los insumos
    tbody.innerHTML = ''; // ⬅️ para que no se pasen de 20  filas

    insumos.forEach(ciclo => { // ⬅️ for Each para recorrer cada fila
      ...
      row.innerHTML = `
        ...
        <td class="cardright__cell">${(ciclo.unit_consumables)}</td>
        <td class="cardright__cell">${ciclo.unitary_value}</td>
        <td class="cardright__cell">${ciclo.total_value}</td>
        <td class="cardright__cell">${ciclo.description_consumables}</td>
      `;
      tbody.appendChild(row);
    });
  }
    
  function mostrarPaginacion(totalinsumos, paginaActual, buscar = '') { // ⬅️ Funcion para las paginas
    const totalPaginas = Math.ceil(totalinsumos / 10);
    ...
    const crearBoton = (num, texto = null, isActive = false) => {
      ...
      btn.addEventListener('click', () => obtenerinsumos(num, buscar)); // 👈 importante
...
// ⬇️ Estoy llamando a la funcion obtenerinsumos, de esta parte todo el codigo
    obtenerinsumos();
// ⬇️Aqui empieza el bloqeu de codigo para buscar⬇️
  ...
  if (button && input) { // ⬅️ Si estamos interarctuando con el boton y el input
    ...
    input.addEventListener("click", function () { // 👈 Si damos click en el input y este tiene algun valor, se vacia
      if (input.value) {
        input.value = "";
        obtenerinsumos(1); // 👈 Al borrar, recarga todo
      }
    });
  } else {
