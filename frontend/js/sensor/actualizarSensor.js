...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Inicializar Choices.js después de llenar las opciones
      ...
      fetch('http://localhost:5501/sensors/id') // ⬅️ hago un fetch para traer la lista de IDS
        .then(res => res.json()) // ⬅️ Aqui estoy trallendo el paquete json
        .then(ids => {
          const todosLosIds = ids.sensores; // Array con todos los IDs
      ...
      cropSelect.addEventListener('change', () => {  // ⬅️ Para cuando se selecciona otro ID se cambie el valor de la variable currentID
        ...
        fetch(`http://localhost:5501/api/sensors/${id}`) // ⬅️ Por medio del ID traigo la columna correspondiente( anteriormente la traje tambien, pero para cargar el select, ahora es para los inputs)
          .then(res => {
          ...
          .then(data => {  // ⬅️ Los muestro por aqui
            ...
            cropForm.estado_sensor.value = data.state_sensor;
    ...
    if (cropForm) {
      cropForm.addEventListener('submit', (e) => { // ⬅️ Cuando se haga el sumbit
        ...
        fetch(`http://localhost:5501/api/sensors/${currentID}`, { // ⬅️ Mandamos con fetch la actualizacion con su id correspondiente
          method: 'PUT',
          body: data
        })
        .then(res => res.json())
